  ...
  return useInfiniteQuery({
    ...
    queryFn: async ({ pageParam = 1 }) => {
      const res = await axios.get(
        `${apiBasePharma}/products/all-products-paginated?page=${pageParam}&paginate=20&sort_by=${sortBy}`
      );
      
      const newProducts: Product[] = (res.data?.data || []).map(
        (p: Record<string, unknown>) => ({
          ...
          product_prices: {
            ...
            ecom_final_selling_price: p.ecom_final_selling_price as number || (p.selling_price as number),
